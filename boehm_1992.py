...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 20 09:08:08 2019
...
def dia2vel(diam, rho_air, nu_air, mass, area, as_ratio=1.0):
  grav = 9.81
  rho_ice = 917.0
  ...
  X = 8.0*mass*grav*rho_air/(np.pi*(eta_air**2)*np.maximum(alpha,np.ones_like(alpha)*1.0)*np.maximum(q**0.25,q)) #reduced to 8.0*mtot*grav*rho/(np.pi*(eta**2)*q**(1/4) = 8.0*mtot*grav*rho/(np.pi*(eta**2)*(area / (np.pi/4.0 * diam**2))**(1/4) for alpha=1 and q<1 (which is usually the case)
